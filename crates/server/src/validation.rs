...
async fn list_rules(
    ...
    let rows = state
        .store
        .list_validation_rules(user.id(), user.can_see_all_workspaces())
        .await?;
    Ok(Json(
        json!({"rules": rows.into_iter().map(rule_json).collect::<Result<Vec<_>, _>>()?}),
    ))
}
...
async fn get_rule(
    ...
    let row = state
        .store
        .get_validation_rule(&id)
        .await?
        .ok_or_else(|| AppError::not_found("validation rule not found"))?;
    if row.owner_user_id != user.id() && !user.can_see_all_workspaces() {
        return Err(AppError::not_found("validation rule not found"));
    }
...
fn validate_rule(body: &SaveRuleBody) -> Result<(), AppError> {
    if body.name.trim().is_empty() {
        return Err(AppError::bad("validation rule name required"));
    }
    if body.keys.iter().all(|v| v.trim().is_empty()) {
...
async fn create_rule(
    ...
    save_rule(&state, &user, None, body).await
}
async fn update_rule(
    ...
    save_rule(&state, &user, Some(&id), body).await
}
async fn delete_rule(
    ...
    state.store.delete_validation_rule(&id, user.id()).await?;
...
async fn list_results(
    ...
    let rows = state
        .store
        .list_validation_results(user.id(), user.can_see_all_workspaces())
        .await?;
...
async fn run_validation(
        ...
        .filter(|v| !v.trim().is_empty())
    {
        let rule = state
            ...
            .await?
            .ok_or_else(|| AppError::not_found("validation rule not found"))?;
        if rule.owner_user_id != user.id() && !user.can_see_all_workspaces() {
            return Err(AppError::not_found("validation rule not found"));
        }
        if rule.active == 0 {
    ...
    let target =
        crate::access::require_dataset(&state.store, &user, &body.target_data_file_id).await?;
    let source_path = state.store.resolve(&source.stored_path);
    let target_path = state.store.resolve(&target.stored_path);
    if !source_path.is_file() || !target_path.is_file() {
        return Err(AppError::bad("validation requires materialized data files"));
    }
    let source_read = TransformSpec::identity()
        .with_read(source.delimiter.clone(), source.has_header.map(|v| v != 0));
    let target_read = TransformSpec::identity()
        .with_read(target.delimiter.clone(), target.has_header.map(|v| v != 0));
    ...
    let result = state
        ...
        .insert_validation_result(
            user.id(),
            &target.workspace_id,
            body.validation_rule_id.as_deref(),
            None,
            &source.id,
            &target.id,
            report.passed,
            &serde_json::to_string(&report).map_err(|e| AppError::bad(e.to_string()))?,
        )
        .await?;
