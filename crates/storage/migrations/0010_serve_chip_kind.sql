...
CREATE TABLE chips_new (
    ...
    name TEXT NOT NULL,
    kind TEXT NOT NULL CHECK (kind IN ('extract', 'transform', 'load', 'validation', 'sql', 'serve')),
    ...
    CHECK (
        ...
        (kind = 'serve' AND extract_id IS NULL AND transform_id IS NULL AND load_id IS NULL)
    )
);
