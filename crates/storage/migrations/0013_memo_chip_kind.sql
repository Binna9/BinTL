...
CREATE TABLE chips_new (
    ...
    name TEXT NOT NULL,
    kind TEXT NOT NULL CHECK (kind IN ('extract', 'transform', 'load', 'validation', 'sql', 'serve', 'script', 'memo')),
    ...
    CHECK (
        ...
        (kind = 'script' AND extract_id IS NULL AND transform_id IS NULL AND load_id IS NULL) OR
        (kind = 'memo' AND extract_id IS NULL AND transform_id IS NULL AND load_id IS NULL)
    )
);
