...
impl Store {
    pub async fn list_validation_rules(&self, owner_user_id: &str, admin: bool) -> Result<Vec<ValidationRuleRow>, StorageError> {
        let sql = if admin {
    ...
    pub async fn get_validation_rule(&self, id: &str) -> Result<Option<ValidationRuleRow>, StorageError> {
        Ok(sqlx::query_as::<_, ValidationRuleRow>(&format!("SELECT {RULE_COLS} FROM validation_rules WHERE id=?"))
            .bind(id).fetch_optional(&self.pool).await?)
    }

    #[allow(clippy::too_many_arguments)]
    pub async fn save_validation_rule(&self, id: Option<&str>, owner_user_id: &str, name: &str,
        description: &str, keys_json: &str, columns_json: &str, compare_row_count: bool,
        compare_schema: bool, active: bool) -> Result<ValidationRuleRow, StorageError> {
        ...
        self.get_validation_rule(&id).await?.ok_or_else(|| StorageError::NotFound("validation rule not found".into()))
    }

    pub async fn delete_validation_rule(&self, id: &str, owner_user_id: &str) -> Result<(), StorageError> {
        let result = sqlx::query("DELETE FROM validation_rules WHERE id=? AND owner_user_id=?")
            .bind(id).bind(owner_user_id).execute(&self.pool).await?;
        if result.rows_affected() == 0 { return Err(StorageError::NotFound("validation rule not found".into())); }
        Ok(())
    }

    #[allow(clippy::too_many_arguments)]
    pub async fn insert_validation_result(&self, owner_user_id: &str, workspace_id: &str,
        rule_id: Option<&str>, step_id: Option<&str>, source_id: &str, target_id: &str,
        passed: bool, report_json: &str) -> Result<ValidationResultRow, StorageError> {
        require_config_json(report_json)?;
        let id = Uuid::new_v4().to_string();
        sqlx::query("INSERT INTO validation_results
            (id, owner_user_id, workspace_id, validation_rule_id, execution_step_id,
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
            .bind(&id).bind(owner_user_id).bind(workspace_id).bind(rule_id).bind(step_id)
            .bind(source_id).bind(target_id).bind(i64::from(passed)).bind(report_json)
            .bind(now_rfc3339()).execute(&self.pool).await?;
        sqlx::query_as::<_, ValidationResultRow>(&format!("SELECT {RESULT_COLS} FROM validation_results WHERE id=?"))
            .bind(id).fetch_one(&self.pool).await.map_err(Into::into)
    }

    pub async fn list_validation_results(&self, owner_user_id: &str, admin: bool) -> Result<Vec<ValidationResultRow>, StorageError> {
        let sql = if admin {
