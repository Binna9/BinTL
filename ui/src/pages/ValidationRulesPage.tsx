...
export function ValidationRulesPage() {
  ...
  return <PageShell fill><PageHeader title={t.rulesTitle} description={t.rulesDescription} iconName="validationRules" actions={<Button variant="primary" onClick={() => open()}><Plus className="size-4" />{t.newRule}</Button>} />
    <Panel fill>
      <PanelHeader title={t.rules} description={t.rulesHint} />
      <PanelBody className="flex min-h-0 flex-1 flex-col overflow-auto p-0">
      {rules.length === 0 ? <EmptyState icon={<NavIcon name="validationRules" />} title={t.noRules} hint={t.noRulesHint} /> : <div className="divide-y divide-border">{paging.items.map((rule) => <div key={rule.id} className="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-4 hover:bg-subtle/35"><div><div className="flex items-center gap-2"><strong className="text-sm text-text">{rule.name}</strong><span className="rounded-full bg-subtle px-2 py-0.5 text-[10px] text-text-tertiary">v{rule.revision}</span>{!rule.active ? <span className="text-xs text-warning">{t.inactive}</span> : null}</div><p className="mt-1 text-xs text-text-secondary">{rule.description || t.noDescription}</p><p className="mt-2 font-mono text-[11px] text-text-tertiary">{t.keys}: {rule.keys.join(", ")} · {t.columns}: {rule.columns.join(", ") || t.allCommonColumns}</p></div><div className="flex gap-1"><Button variant="quiet" onClick={() => open(rule)}><Pencil className="size-3.5" />{messages.common.edit}</Button><Button variant="quiet" onClick={() => void remove(rule)}><Trash2 className="size-3.5" />{messages.common.delete}</Button></div></div>)}</div>}
      </PanelBody>
