...
export function ValidationResultsPage() {
  ...
  useEffect(() => { void Promise.all([validationApi.listResults(), validationApi.listRules(), datasetApi.list()]).then(([a, b, c]) => { setResults(a.results); setRules(b.rules); setFileNames(new Map(c.datasets.map((file) => [file.id, file.filename]))); }).catch((e) => toastError(t.loadError, e)); }, []);
  const names = useMemo(() => new Map(rules.map((r) => [r.id, r.name])), [rules]);
  const paging = usePagination(results);
  return <PageShell fill><PageHeader title={t.resultsTitle} description={t.resultsDescription} iconName="validationResults" /><Panel fill><PanelHeader title={t.results} description={t.resultsHint} /><PanelBody className="flex min-h-0 flex-1 flex-col overflow-auto p-0">{results.length === 0 ? <EmptyState icon={<NavIcon name="validationResults" />} title={t.noResults} hint={t.noResultsHint} /> : <div className="divide-y divide-border">{paging.items.map((result) => <div key={result.id} className="grid grid-cols-[8rem_minmax(14rem,1fr)_repeat(4,5rem)_auto] items-center gap-3 px-5 py-3 text-xs"><span className={result.passed ? "font-bold text-success" : "font-bold text-danger"}>{result.passed ? t.passed : t.failed}</span><div className="min-w-0"><p className="truncate font-semibold text-text">{result.validation_rule_id ? names.get(result.validation_rule_id) || t.deletedRule : t.customRule}</p><p className="mt-1 truncate text-[11px] text-text-tertiary">{fileNames.get(result.source_data_file_id) || result.source_data_file_id} → {fileNames.get(result.target_data_file_id) || result.target_data_file_id}</p></div><span>{t.sourceRows}<strong className="ml-1">{result.report.source_rows}</strong></span><span>{t.targetRows}<strong className="ml-1">{result.report.target_rows}</strong></span><span>{t.missing}<strong className="ml-1">{result.report.missing_keys}</strong></span><span>{t.mismatch}<strong className="ml-1">{result.report.mismatched_rows}</strong></span><Button variant="quiet" onClick={() => setSelected(result)}><Eye className="size-3.5" />{messages.common.view}</Button></div>)}</div>}</PanelBody><PaginationBar page={paging.page} pageCount={paging.pageCount} pageSize={paging.pageSize} total={paging.total} start={paging.start} end={paging.end} onPageChange={paging.setPage} onPageSizeChange={paging.setPageSize} /></Panel>
