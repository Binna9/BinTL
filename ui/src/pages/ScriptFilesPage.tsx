...
export function ScriptFilesPage() {
  const { messages } = useLanguage();
  const t = messages.scriptFiles;
  ...
  const files = useMemo(
    () => datasets.filter((item) => item.kind === "script"),
    [datasets],
  );
  ...
  async function deleteSelected() {
    ...
    const confirmed = await showConfirm(
      ...
      t.deleteConfirmMessage(selected.length),
  ...
  return (
    <PageShell>
        ...
        <SplitLayout className="min-h-0 flex-1" defaultSizes={[layout.split.sidebar]}>
          ...
          <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
            ...
            <DataGrid
              ...
              empty={visibleFiles.length === 0 ? <EmptyState icon={<NavIcon name="scriptFiles" />} title={files.length === 0 ? messages.empty.scriptFiles : messages.workspace.fileCatalogEmpty} hint={files.length === 0 ? messages.empty.scriptFilesHint : undefined} /> : undefined}
            >
      ...
      <AppDialog
        ...
        title={previewing?.filename ?? t.previewTitle}
          ...
          <div className="min-h-0 min-w-0 flex-1 overflow-hidden p-4">
            {previewLoading || !preview ? (
              <div className="grid h-full min-h-64 place-items-center text-sm text-text-tertiary">
                {previewLoading ? t.previewLoading : t.previewEmpty}
              </div>
            ) : (
              <DataGrid
